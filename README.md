高性能2.5D SLG基础框架，基于视口渲染与缓冲池机制，实现万级单位同屏流畅渲染及计算。框架内置直线/自定义路径行军线,支持任意轨迹（贴地/飞行等）、城池等丰富基础元素，并集成完整手势交互系统，为超大地图下的策略体验提供稳定高效的底层支撑。

1.可以跟自己的UI框架，资源系统结合，自定义加载方式，目前采用UniTask异步加载，地图支持2D/2.5D，地图物件支持2D/3D,支持通用UI交互。
