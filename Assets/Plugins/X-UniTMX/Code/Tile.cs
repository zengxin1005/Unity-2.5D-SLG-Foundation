...
namespace X_UniTMX
{
	...
	public class Tile
	{
        ...
        //add by zengxin
        private static Queue<Tile> _recycleTiles = new Queue<Tile>();
        
        public SpriteRenderer TileRenderer { get; protected set; }
        ...
        public Tile(TileSet tileSet, Sprite sprite, int OriginalID, int mapTileWidth)
        {
            if (tileSet == null)
                throw new ArgumentNullException("tileSet");
		...
		{
			// Create Sprite
            TileSprite = Sprite.Create(TileSet.Texture, Source, pivot, MapTileWidth, (uint)(TileSet.Spacing * 2), SpriteMeshType.FullRect);
			TileSprite.name = OriginalID.ToString();
		}
        ...
        public void CreateTileObject(string objectName, Transform parent, string sortingLayerName, int sortingLayerOrder, Vector3 position, List<Material> materials, float opacity = 1.0f)
        {
            if (TileGameObject == null) {
	            ...
	            TileGameObject.name = objectName;
            }


            TileGameObject.transform.parent = parent;
            if (TileRenderer == null) {
                TileRenderer = TileGameObject.AddComponent<SpriteRenderer>();
            }
            ...
            TileGameObject.transform.localScale = Vector2.one;
        ...
        public virtual Tile Clone()
        {
            ...
            {
               t = new Tile(TileSet, Source, OriginalID, Properties);
            }
            else 
            {
                t = _recycleTiles.Dequeue();
                if (t.TileGameObject != null)
                    t.TileGameObject.SetActive(true);

                t.OriginalID = OriginalID;
                t.CurrentID = OriginalID;
                t.TileSet = TileSet;
                t.Source = Source;
                t.Properties = Properties ?? new PropertyCollection();
                t.Color = Color.white;
                //SpriteEffects = new X_UniTMX.SpriteEffects();
                if (t.MapTileWidth <= 0)
                    t.MapTileWidth = TileSet.TileWidth;
            }
        ...
        public string GetPropertyAsString(string property)
        {
			if (Properties == null)
				return string.Empty;
            if (!TileSet.TileProperties.ContainsKey(this.OriginalID))
            {
                return string.Empty;
            }
            var propertyCollection = this.TileSet.TileProperties[this.OriginalID];
            return propertyCollection.GetPropertyAsString(property);
        }
        /// <summary>
