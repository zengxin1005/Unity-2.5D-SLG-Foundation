...
namespace X_UniTMX
{
	...
	{
        public delegate  UniTask<Texture2D> LoaderTexture(string path);
        public delegate UniTask<Sprite> LoaderSprite(string path);
        ...
        protected Dictionary<int, TileImage> m_tileImage = null;
        protected string m_sheet = null;
		/// <summary>
		...
		protected TileSet(NanoXMLNode node, Map map, int firstGID = 1)
		{
			...
			foreach (NanoXMLNode subNode in node.SubNodes)
			{
                ...
                if (null == m_tileImage)
                    m_tileImage = new Dictionary<int, TileImage>();
                ...
                m_tileImage.Add(id, tileImage);
                NanoXMLNode subImgNode = subNode["image"];
                if (null != subImgNode)
                {
                    tileImage.Width = int.Parse(subImgNode.GetAttribute("width").Value, CultureInfo.InvariantCulture);
                    tileImage.Height = int.Parse(subImgNode.GetAttribute("height").Value, CultureInfo.InvariantCulture);
                    tileImage.Source = subImgNode.GetAttribute("source").Value;
                }
        ...
        public TileSet(NanoXMLNode node, string mapPath, Map map, LoaderTexture textureLoader, LoaderSprite spriteLoader, Action<TileSet> onFinishedLoadingTileSet = null, int firstID = 1) 
			: this(node, map, firstID)
		{
            ...
            if(null != m_tileImage)
            {
                ...
                BuildTilesUseAtlas(mapPath, textureLoader, spriteLoader).Forget();
                return;
            }
		}

        async UniTaskVoid BuildTilesUseAtlas(string mapPath, LoaderTexture textureLoader, LoaderSprite spriteLoader)
        {
            foreach (int index in m_tileImage.Keys)
            {
                TileImage imge = m_tileImage[index];
                string src = imge.Source;
                string ext = Path.GetExtension(src);
                string path = src.Substring(0, src.Length - ext.Length);

                if(this.Texture == null)
                {
                    string texPath = path.Substring(0, src.IndexOf("/"));

                    this.Texture = await textureLoader(mapPath + texPath);
                }

                Sprite sp = await spriteLoader(mapPath + path);
                if(sp == null)
                {
                    continue;
                }
                Tiles.Add(index, new Tile(this, sp, index, imge.Width));
            }
