...
namespace X_UniTMX
{
	...
	public class Map
	{
        ...
        //像素宽度
        public int PixelWidth { get { return _pixelWidth; } }
        //像素高度
        public int PixelHeight { get { return _pixelHeight; } }

        public Vector2 ViewprotSize { get; set; }
		...
		/// <summary>
		/// UniTask 形式的"地图完全加载完成"信号(继续加载完所有 tileset + 填好 Layers[] 后触发)。
		/// 用 UniTaskCompletionSource 是为了避免"builder 同步跑完"导致 Action 订阅错过的 race condition —
		/// 无论 builder 同步还是异步完成,await WhenLoaded() 都能正确等到。
		/// </summary>
		private readonly UniTaskCompletionSource _loadedTcs = new UniTaskCompletionSource();

		/// <summary>
		/// 异步等待地图完全加载完成(TileSets[] + Layers[] 都填好)。
		/// </summary>
		public UniTask WhenLoaded() => _loadedTcs.Task;

		#region Custom Properties
		/// <summary>
		/// Custom Object Type for Collider Objects
		/// </summary>
		protected const string Object_Type_Collider = "Collider";
		/// <summary>
		/// Custom Object Type for Trigger Objects
		/// </summary>
		protected const string Object_Type_Trigger = "Trigger";
		/// <summary>
		/// Custom Object Type for Objects with no collider
		/// </summary>
		protected const string Object_Type_NoCollider = "NoCollider";
		...
		/// </summary>
		protected const string Property_PrefabName = "-prefab";
		/// <summary>
        ...
        public Map(TextAsset mapText, string mapPath, GameObject parent,
					Material baseTileMaterial, int sortingOrder, TileSet.LoaderTexture textureLoader, TileSet.LoaderSprite spriteLoader)
		{

            _tileObjectEllipsePrecision = 16;//tileObjectEllipsePrecision;
			_simpleTileObjectCalculation = true;
			_clipperArcTolerance = 0.25;//clipperArcTolerance;
			_clipperDeltaOffset = 0;//clipperDeltaOffset;
			_clipperEndType = ClipperLib.EndType.etClosedPolygon;//clipperEndType;
			_clipperJoinType = ClipperLib.JoinType.jtRound;//clipperJoinType;
			_clipperMiterLimit = 2.0;//clipperMiterLimit;
			...
			GlobalMakeUniqueTiles = true;
			BaseTileMaterial = baseTileMaterial;
			_mapPath = Path.GetDirectoryName(mapPath);
			//OnMapFinishedLoading = onMapFinishedLoading;

            Initialize(document, textureLoader, spriteLoader);
		}
		#endregion
        ...
        void Initialize(NanoXMLDocument document, TileSet.LoaderTexture textureLoader, TileSet.LoaderSprite spriteLoader)
		{
			...
			foreach (NanoXMLNode node in MapNode.SubNodes)
			{
                ...
                new TileSet(node, _mapPath, this, textureLoader, spriteLoader, OnFinishedLoadingTileSet);
			}


            _pixelWidth = this.Width * this.TileWidth;
            _pixelHeight = this.Height * this.TileHeight;
		}
		...
		void ContinueLoadingTiledMapAfterTileSetsLoaded()
		{
			...
			foreach (NanoXMLNode layerNode in MapNode.SubNodes)
			{
				...
				if (layerNode.Name.Equals("layer"))
				{
                    ...
                    TileLayer content = new TileLayer(layerNode, this, layerDepth, makeUnique, materials);
                    content.SortingOrderStart = i * this.Height * 2;
            ...
            TimeSpan diff = ts2.Subtract(ts1).Duration();
            Debug.LogFormat("Map ContinueLoadingTiledMapAfterTileSetsLoaded {0}", diff.TotalSeconds);

            // 通知所有 await WhenLoaded() 的协程:TileSets[] + Layers[] 都已填好,
            // 即"地图 xml + 图集 png + sprite 全部加载完成"。
		...
		public void ApplyCustomProperties(GameObject gameObject, MapObject obj)
		{
			...
			{
                Type type = Type.GetType(Property_AddComponent + c);
                gameObject.AddComponent(type);
                //UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(gameObject, "Assets/Externals/X-UniTMX/Code/Map.cs (2684,5)", obj.GetPropertyAsString());
				c++;
			}
			c = 1;
        ...
        public void SetupTile(int x, int y, bool needLayerActiveInHierarchy = true)
        {
            ...
            {
                TileLayer tileLayer = this.Layers[i] as TileLayer;

                if(needLayerActiveInHierarchy && !tileLayer.LayerGameObject.activeInHierarchy)
                {
                    continue;
                }
                tileLayer.SetupTile(x, y);
            }
        }

        public TileLayer GetLayer(int index)
        {
            return this.Layers[index] as TileLayer;
        }
