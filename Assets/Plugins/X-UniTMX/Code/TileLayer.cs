...
namespace X_UniTMX
{
	...
	public class TileLayer : Layer
	{
            ...
            : base(node)
		{
            ...
            this.LazyInitialize(map, Data, materials);
        ...
        {
            int dataLen = dataNode.Value.Length;
            ...
            while (true)
            {
                int readSize = data.Read(buffer, 0, bufferSize);
                if (readSize <= 0)
                    break;
            ...
            //Debug.LogFormat("TileLayer {0}", diff.TotalSeconds);
        }
		...
		private void GenerateLayer()
		{
			...
			LayerGameObject.transform.localPosition = new Vector3(0, 0, 0);
			LayerGameObject.isStatic = true;

			LayerGameObject.SetActive(Visible);
		}

        private void LazyInitialize(Map map, uint[] data, List<Material> materials)
        {
            BaseMap = map;
            Tiles = new TileGrid(Width, Height, false);
		...
		{
            string fullName = Name + "[" + x + ", " + y + "]";
			t.CreateTileObject(fullName,
				LayerGameObject.transform,
				Name,
                this.SortingOrderStart,
		...
		void CreateLayerMesh(int startX, int endX, int startY, int endY, int directionX, int directionY)
		{
            ...
            int test = LayerTileSets.Count;
			foreach (var tileSet in LayerTileSets)
			{
                string name = string.Concat(Name, "_", tileSet.Name, "_", count.ToString());
				...
				Tile t;
				for (int x = startX; startX > endX ? x > endX : x < endX; x += directionX)
				{
					for (int y = startY; startY > endY ? y > endY : y < endY; y += directionY)
					{
						t = Tiles[x, y];
						if (t != null && t.TileSet.Equals(tileSet))
						{
							...
							if (vertexCount >= maxVerticesNumber)
							{
								...
								vertexCount = 0;
                                string strName = string.Concat(Name, "_", tileSet.Name, "_", count.ToString());
                                tileSetGameObject = new GameObject(strName);
        ...
        public void SetupTile(int x, int y)
        {
	        ...
	        if (index < 0 || index >= Width * Height)
		        return;
            if (Tiles.HasTile(x, y))
                return;
            uint id = Data[y * Width + x];
            Tile t = null;
            if(BaseMap.Tiles.TryGetValue((int)id, out t))
            {
                t = t.Clone();
                CreateTileGameObject(t, x, y);
                Tiles.AddTile(x, y, t);
        ...
        public bool HasPropertyValue(int x, int y, string propetyName, string propertyValue)
        {
            Tile tile = Tiles.GetTile(x, y);
            if (null == tile)
                return false;
            return tile.GetPropertyAsString(propetyName).Equals(propertyValue);
        }

        public string GetProperty(int x, int y, string propetyName)
        {
            Tile tile = Tiles.GetTile(x, y);
            if (null == tile)
                return "";
            return tile.GetPropertyAsString(propetyName);
        }
